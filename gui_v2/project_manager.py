...
"""
    ...
    def _count_project_files(self, project_folder):
        ...
        # Count prediction files (root + run subfolders)
        root_predictions = glob.glob(os.path.join(project_folder, "predictions", "*.csv"))
        subfolder_predictions = glob.glob(os.path.join(project_folder, "predictions", "*", "*.csv"))
        counts["prediction_files"] = len(root_predictions) + len(subfolder_predictions)
    ...
    def save_metadata_config(self, metadata_config):
        """Save metadata extraction config to project settings.

        Args:
            metadata_config: MetadataConfig instance, or None to clear
        """
        if not self.current_project_config:
            return False

        if metadata_config is not None:
            self.current_project_config.setdefault("settings", {})["metadata_config"] = metadata_config.to_dict()
        else:
            self.current_project_config.setdefault("settings", {})["metadata_config"] = None
    ...
    def get_metadata_config(self):
        """Load saved metadata extraction config from project settings.

        Returns:
            MetadataConfig instance if previously saved, or None
        """
        if not self.current_project_config:
            return None

        data = self.current_project_config.get("settings", {}).get("metadata_config")
        if not data:
            return None

        try:
            from core.feature_extraction import MetadataConfig
            return MetadataConfig.from_dict(data)
        except Exception:
            return None

    def save_selected_features(self, selected_features):
        """Save selected feature list to project config.

        Args:
            selected_features: List of feature column names to use for training.
                Pass None to clear (use all features).
        """
        if not self.current_project_config:
            return False

        self.current_project_config.setdefault("settings", {})["selected_features"] = selected_features
        self.current_project_config["last_modified"] = datetime.now().isoformat()
    ...
    def get_selected_features(self):
        """Load saved selected features from project config.

        Returns:
            List of feature names if previously saved, or None (meaning use all).
        """
        if not self.current_project_config:
            return None

        return self.current_project_config.get("settings", {}).get("selected_features")
