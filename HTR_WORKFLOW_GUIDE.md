...
└───────────────────────────────┬─────────────────────────────────┘
              ...
              ▼                         ▼
     ┌────────────────┐        ┌────────────────────┐
     │ TUNE PARAMETERS│        │ Skip to Feature    │
     │ - Load H5/video│        │ Extraction         │
     │ - Adjust params│        │                    │
     │ - Visual verify│        │                    │
     │ - Save params  │        │                    │
     └────────┬───────┘        └──────┬─────────────┘
              ...
              └───────────┬───────────┘
                ...
                └──────┬─────────────┘
                       ...
                       ▼
        ┌──────────────────────────────┐
        │ Have trained ML model ready? │
        └─────────┬────────────────────┘
                  │
      ┌───────────┴───────────┐
      │ YES                   │ NO - Need to train model
      │                       │
      ▼                       ▼
┌─────────────┐    ┌──────────────────────────┐
│ Skip to     │    │ Have ground truth labels?│
│ BATCH       │    └───────┬──────────────────┘
│ PROCESSING  │            │
└─────────────┘  ┌─────────┴─────────┐
                 │ YES               │ NO - Need to label
                 │                   │
                 ▼                   ▼
        ┌────────────────┐    ┌─────────────────────┐
        │ Skip to ML     │    │ LABEL GROUND TRUTH  │
        ...
        └────────────────┘    │ - Note HTR times    │
                              ...
                              └──────┬──────────────┘
                                     ...
                                     ▼
                         ┌───────────────────────────┐
                         │ Labeling complete/ready   │
                         │ for first model training? │
                         └──────┬────────────────────┘
                                │
                                ▼
                    ┌───────────────────────────┐
    ...
    ▼                   ▼
┌───────────────────┐  ┌─────────────────────┐
│ ITERATIVE REFINE  │  │ BATCH PROCESSING    │
│ - Review misclass │  │ - Load H5 files     │
│ - Watch videos    │  │ - Extract features  │
│ - Fix labels:     │  │ - Run predictions   │
│   * Was miss real?│  │ - Generate reports  │
│   * Was FP real?  │  │                     │
│ - Add more labels?│  │ OR                  │
│                   │  │                     │
└────┬──────────────┘  │ INCREMENTAL BATCH   │
     │                 │ - Process new files │
     └──► RETRAIN      │ - Update reports    │
          (back to     │                     │
          ML TRAINING) └──────────┬──────────┘
                                  │
                                  ▼
